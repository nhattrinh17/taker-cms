...
function Table(props: {
  ...
  const { data = [], columnToName, columnEdit, columnDelete, columnRestore, backgroundColor, textColor, fontSizeData, backgroundColorHeader = '#526dfa', handleEdit, handleDelete, handleRestore, pageSort, handleSort, textColorHeader = '#fff', columnNotShow = [], handleClickRow, moreColumnsOptions } = props;
  ...
  if (columnToName) {
    columnNames = Object.keys(columnToName);
  }
            ...
            <tr className={cx('table__information')}>
              {columnNames?.map((columnName, index) => (
                <th key={index} className={cx('thead__column')}>
                  <p className="w-max text-center block px-6">{columnToName[columnName]}</p>
                </th>
              ))}
